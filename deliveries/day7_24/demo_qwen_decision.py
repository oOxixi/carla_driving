...
def summarize_result(r: dict) -> dict:
    ...
    return {
        ...
        "qwen_action": action,
        "qwen_confidence": raw.get("confidence", "?") if isinstance(raw, dict) else "?",
        "qwen_reason": raw.get("reason_zh", "") if isinstance(raw, dict) else "",
...
def main():
    ...
    print(f"  Prompt版本: day22_v2")
    print(f"  Day22动作白名单 (5个): START, STOP, SLOW_DOWN, SET_SPEED, EMERGENCY_STOP")
    print(f"  Day23动作白名单 (10个): {sorted(ALLOWED_ACTIONS)}")
    ...
    for i, r in enumerate(results[:8]):
        ...
        print(f"      Qwen输出: {s['qwen_action']} (conf={s['qwen_confidence']})")
