...
```python
# demo_full_chain.py 核心导入
from deliveries.day7_24.demo_visual import spawn_traffic      # CARLA场景生成
from deliveries.day7_24.demo_qwen_decision import (
    load_day22_results,   # 加载Day22真实Qwen运行时数据
    summarize_result,     # 解析案例摘要
)
```

**视觉识别（demo_visual.py）的关键实现：**

```python
def spawn_traffic(world, ego_transform):
    """基于ego实际位置生成前车和行人."""
    loc = ego_transform.location
    fwd = ego_transform.rotation.get_forward_vector()   # 前方单位向量
    right = ego_transform.rotation.get_right_vector()    # 右侧单位向量
    ...
    # 行人: 在ego前方6-10m、左右2-4m范围内尝试spawn
    ...
    # ONNX检测: YOLO11n, 置信度0.35
    detector = OnnxYoloDetector(model_path=..., confidence_threshold=0.35)
    detections = detector.detect_rgb(img)

    # DetectedObject是dataclass，使用属性访问
    for d in detections:
        cls = d.class_name            # "person" / "car"
        conf = d.confidence           # 0.89
        bbox = d.bbox_xyxy_norm       # (x1,y1,x2,y2) 归一化0-1 → 需反归一化
```

**Qwen决策（demo_qwen_decision.py）的数据流：**

```python
# 加载Day22真实运行时数据 (12案例, prompt=day22_v2)
results = load_day22_results()  # → integration/day22/day22_qwen_runtime_results.json

# 每个案例结构:
{
    "case": "ttc_emergency",
    "qwen_raw_output": '{"action":"SLOW_DOWN","reason_zh":"前方距离不足",...}',
    "qwen_validation": {"format_valid": True, "hallucination_flags": [...]},
    "final_decision": {"action":"EMERGENCY_STOP","decision_source":"SAFETY_RULE"},
}
```
...
| 脚本 | 依赖 | 独立运行 | 说明 |
|------|------|:--:|------|
| `demo_qwen_decision.py` | 无 | ✅ | 纯数据展示，不依赖CARLA |
| `demo_visual.py` | CARLA + ONNX模型 | ✅ | 独立采集+检测 |
| `demo_full_chain.py` | 上面两个脚本 + CARLA | ✅ | 复用 spawn_traffic + load_day22_results |
