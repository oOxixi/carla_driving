...
清单的所有必需字段必须是非空字符串；`git_sha` 使用完整 40 位十六进制值，
`config_id` 与实际加载模型的配置严格匹配。A3 Gate 声明不替代 B2/B3 最终验收。
...
导出同时刷新同目录的 `model_structure.json`、`flops_report.json`，保持模型摘要与
来源版本一致。复现发布产物时先检出报告中的 `source_git_sha`；默认导出记录当前 HEAD。
交付流程回归另运行 `py -3.12 -m pytest -q challenge/tests/test_delivery.py`。
