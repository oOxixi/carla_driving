...
class RouteManager:
    ...
    def rejoin_route(
        ...
        *,
        ...
        maximum_connector_m: float = 180.0,
    ) -> GlobalRoute:
        """Reconnect to a forward point and retain the declared mission suffix.

        Coverage missions may intentionally revisit map topology. Regenerating
        the remaining distance from an empty visit history can therefore trap
        recovery in a nearby roundabout. This method plans only a bounded
        connector and then resumes the already validated forward route.
        """
        values = (route_s, target_speed_mps, lookahead_m, maximum_connector_m)
        if any(
            type(value) not in (int, float)
            or isinstance(value, bool)
            or not math.isfinite(float(value))
            ...
            raise ValueError("route rejoin values must be finite numbers")
        if route_s < 0.0 or target_speed_mps < 0.0:
            raise ValueError("route_s and target_speed_mps must be non-negative")
        if lookahead_m <= 0.0 or maximum_connector_m <= 0.0:
            raise ValueError("route rejoin distances must be positive")

        base_cumulative = cumulative_distances_m(route.reference.points_xy_m)
        last_error: RoutePlanningError | None = None
        for multiplier in (1.0, 2.0, 4.0):
            merge_s_m = min(
                route.total_length_m,
                float(route_s) + float(lookahead_m) * multiplier,
            )
            merge_index = min(
            ...
            try:
                connector = self.replan(
                    current,
                    route.waypoints[merge_index].transform,
                    float(target_speed_mps),
                )
            except RoutePlanningError as error:
                last_error = error
                continue
            allowed_connector_m = min(
                float(maximum_connector_m),
                float(lookahead_m) * multiplier * 2.0 + 30.0,
            )
            if connector.total_length_m > allowed_connector_m:
                last_error = RoutePlanningError(
                    "ROUTE_RECOVERY_CONNECTOR_TOO_LONG",
                    f"connector length {connector.total_length_m:.2f} m exceeds "
                    f"the {allowed_connector_m:.2f} m recovery bound",
                )
                continue

            points = (
                *connector.reference.points_xy_m[:-1],
                *route.reference.points_xy_m[merge_index:],
            )
            cumulative = cumulative_distances_m(points)
            gaps = tuple(
            ...
            if maximum_gap_m > self.maximum_gap_m:
                last_error = RoutePlanningError(
                    "ROUTE_DISCONTINUOUS",
                    f"rejoin gap {maximum_gap_m:.2f} m exceeds "
                    f"the {self.maximum_gap_m:.2f} m route bound",
                )
                continue
            connector_count = len(connector.reference.points_xy_m) - 1
            samples = (
                *(
                    replace(sample, s_m=cumulative[index])
                    for index, sample in enumerate(connector.samples[:-1])
                ),
                *(
                    replace(sample, s_m=cumulative[connector_count + index])
                    for index, sample in enumerate(route.samples[merge_index:])
                ),
            )
            repeated = len(points) - len({
            ...
            validation = RouteValidation(
                ...
                destination_error_m=math.dist(points[-1], route.destination_xy_m),
                repeated_sample_count=repeated,
                junction_count=sum(
                    1 for previous, sample in zip(samples, samples[1:])
                    if sample.is_junction and not previous.is_junction
                ),
            )
            metadata = dict(route.reference.metadata)
            metadata.update({
                "planning_reason": "OFF_ROUTE_REJOIN",
                "source_route_id": route.reference.route_id,
                "rejoin_route_s_m": base_cumulative[merge_index],
                "recovery_connector_m": connector.total_length_m,
                "validation": validation.to_dict(),
            })
            fingerprint = hashlib.sha256(
                (
                    f"{route.reference.route_id}|{base_cumulative[merge_index]:.2f}|"
                    + "|".join(
                        f"{x_m:.2f},{y_m:.2f}"
                        for x_m, y_m in points[:64]
                    )
                ).encode("utf-8")
            ).hexdigest()[:16]
            return GlobalRoute(
                reference=RouteReference(
                    ...
                    float(target_speed_mps),
                    route_id=f"rejoin-{fingerprint}",
                    metadata=metadata,
                ),
                samples=samples,
                validation=validation,
                start_xy_m=connector.start_xy_m,
                destination_xy_m=route.destination_xy_m,
                waypoints=(
                    *connector.waypoints[:-1],
                    *route.waypoints[merge_index:],
                ),
            )

        detail = (
            "no bounded connector can reach a forward point on the mission route"
            if last_error is None else last_error.detail
        )
        raise RoutePlanningError("ROUTE_RECOVERY_REJOIN_FAILED", detail)
