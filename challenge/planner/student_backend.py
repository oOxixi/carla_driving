        ...
        if weights is not None:
            ...
            if weights_manifest is not None:
                manifest = validate_weight_manifest(
                    ...
                    expected_config_id=self.model.config.config_id,
                )
...
def validate_weight_manifest(
    ...
    *,
    ...
    expected_config_id: str = StudentModelConfig().config_id,
) -> dict[str, Any]:
    path = Path(weights)
    ...
    for key in sorted(required):
        if not isinstance(manifest[key], str) or not manifest[key].strip():
            raise ValueError(f"weight manifest {key} must be a non-empty string")
    if not re.fullmatch(r"[0-9a-fA-F]{40}", manifest["git_sha"]):
        raise ValueError("weight manifest git_sha must be a full 40-character Git SHA")
    if manifest.get("model_id") != expected_model_id:
        raise ValueError("weight manifest model_id does not match Student structure")
    if manifest["config_id"] != expected_config_id:
        raise ValueError("weight manifest config_id does not match Student configuration")
