...
|`safety_state.py`|RGB 类别、LiDAR 前向距离、闭合速度/TTC 的保守融合，有效性/来源标记与失效全制动|
...
`ConservativeSensorFusion.update()` 是 C 面向第一组视觉输出的冻结接口。第一组只需提供同帧 `VisualObservation(frame, valid, object_class, confidence, source)`；C 将其与 LiDAR 前向距离及跟踪速度（若有）组合为 `SafetyStateSummary`。摘要显式包含 `visual_valid`、`lidar_valid`、`fused_valid`、`source_by_field`、`front_distance_m`、`closing_speed_mps`、`ttc_s`、建议动作和原因。

- 未接入检测器时必须使用 `VisualObservation.unavailable(frame)`；`object_class` 保持 `None`，不得依据 LiDAR 距离猜测类别。
- 有 LiDAR 距离但无可靠视觉语义时使用 `LIDAR_ONLY`，距离/TTC 仍参与 C/D 控制。
- LiDAR 帧无效，或 RGB 已确认危险目标但 LiDAR 没有可用距离时，摘要为 `FULL_BRAKE`；集成 runner 将其作为锁存安全告警送入 D，最终下发全制动。
- `CarlaPerceptionBridge(..., detector=OnnxYoloDetector(...))` 可直接使用从 GitHub `ef7950a` 兼容合入的 ONNX 检测器；`visual_provider=provider` 仍可接第一组的其他检测器适配函数。未配置检测器时会如实记录 `RGB_DETECTOR_UNAVAILABLE`。
...
`SafetyStateParameters` 默认值：视觉有效阈值 `0.60`，提醒/紧急距离 `10/5 m`，提醒/紧急 TTC `2.5/1.5 s`，连续观测最大间隔 `0.30 s`，异常制动 `1.0`。这些值会随 `tools/validate_c_role.py` 的证据摘要一起冻结输出。
...
`test_safety_state.py` 覆盖 RGB/LiDAR 同帧融合、基于 LiDAR 时序差分的闭合速度/TTC、视觉缺失不编造、低置信度无效化、LiDAR 失效全制动、RGB/LiDAR 冲突全制动和 episode reset。`integration/tests/test_carla_perception.py` 还验证真实桥接入口、畸形 LiDAR fail-closed 以及 C 安全摘要输出。

截止日前的 C 证据包可在仓库根目录生成：

```powershell
python tools\validate_c_role.py
```

输出位于 `artifacts/C_role_validation/`：`front_distance_ttc_samples.csv`、`stop_curve.csv`、`fault_injection.json`、`summary.json`。这是不依赖 CARLA 的确定性验收；实时道路效果仍须按计划运行前车、行人、红灯和低 TTC 场景，并保留 CARLA 日志/视频。
