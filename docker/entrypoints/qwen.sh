...
readonly launch_id="${QWEN_LAUNCH_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
readonly output_root="${QWEN_OUTPUT_ROOT:-/output/runs}"
readonly startup_log="$output_root/qwen-startup-$launch_id.log"
...
monitor_startup() {
  while true; do
    ...
    if [[ -n "$model_response" ]]; then
      printf 'vllm_models=%s\n' "$model_response" | tee -a "$vllm_log" >> "$startup_log"
    fi

    if grep -Fq "$REQUIRED_MODEL" "$vllm_log" 2>/dev/null \
...
exec vllm serve /models/qwen \
  ...
  "${extra_args[@]}" > >(tee -a "$vllm_log" | tee -a "$startup_log") 2>&1
