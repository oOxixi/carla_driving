# run_official_s2_member3：功能记录

上级模块：[模块说明](../modules/support-tools.md) · 实现：[scripts/run_official_s2_member3.sh](../../../scripts/run_official_s2_member3.sh)
...
- 第 9 行：`qwen_timeout_ms="${QWEN_TIMEOUT_MS:-5000}"`
- 第 10 行：`carla_host="${CARLA_HOST:-127.0.0.1}"`
- 第 11 行：`carla_port="${CARLA_PORT:-2000}"`
- 第 12 行：`log_dir="${S2_LOG_DIR:-artifacts/logs/official_competition}"`
- 第 13 行：`mode="${1:---run}"`
- 第 61 行：`--qwen-image-root "$project_root"`
- 第 69 行：`"$python_executable" "${arguments[@]}"`
- 第 87 行：`--output "${latest_jsonl%.jsonl}.member3.json"`
...
### `scripts/run_official_s2_member3.sh`

来源 SHA256：`c23e4402d1085d259af274b6218fa86ef65e77cd6843ae1a0c963a022d08b8e3`。
