...
上级模块：[模块说明](../modules/support-delivery.md) · 实现：[stop.ps1](../../../stop.ps1)
...
- 第 1 行：`param([ValidateSet('rtx5070','a800-safe','a800-optimized')][string]$Profile = 'rtx5070')`
- 第 3 行：`& docker compose --project-directory $root --env-file (Join-Path $root "config/repro/$Profile.env") -f (Join-Path $root 'docker/compose.yaml') down`
