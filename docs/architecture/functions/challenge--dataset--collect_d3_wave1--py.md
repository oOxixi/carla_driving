# collect_d3_wave1：功能记录

上级模块：[模块说明](../modules/challenge-data.md) · 实现：[challenge/dataset/collect_d3_wave1.py](../../../challenge/dataset/collect_d3_wave1.py)
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 64 行](../../../challenge/dataset/collect_d3_wave1.py#L64)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 71 行](../../../challenge/dataset/collect_d3_wave1.py#L71)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 79 行](../../../challenge/dataset/collect_d3_wave1.py#L79)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 95 行](../../../challenge/dataset/collect_d3_wave1.py#L95)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 111 行](../../../challenge/dataset/collect_d3_wave1.py#L111)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 115 行](../../../challenge/dataset/collect_d3_wave1.py#L115)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 155 行](../../../challenge/dataset/collect_d3_wave1.py#L155)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 190 行](../../../challenge/dataset/collect_d3_wave1.py#L190)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 230 行](../../../challenge/dataset/collect_d3_wave1.py#L230)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 270 行](../../../challenge/dataset/collect_d3_wave1.py#L270)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 288 行](../../../challenge/dataset/collect_d3_wave1.py#L288)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 510 行](../../../challenge/dataset/collect_d3_wave1.py#L510)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 537 行](../../../challenge/dataset/collect_d3_wave1.py#L537)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 573 行](../../../challenge/dataset/collect_d3_wave1.py#L573)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 600 行](../../../challenge/dataset/collect_d3_wave1.py#L600)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 633 行](../../../challenge/dataset/collect_d3_wave1.py#L633)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 691 行](../../../challenge/dataset/collect_d3_wave1.py#L691)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 705 行](../../../challenge/dataset/collect_d3_wave1.py#L705)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 809 行](../../../challenge/dataset/collect_d3_wave1.py#L809)。类型：`FunctionDef`。
...
源码位置：[challenge/dataset/collect_d3_wave1.py 第 871 行](../../../challenge/dataset/collect_d3_wave1.py#L871)。类型：`FunctionDef`。
...
- `collect_dataset`，第 866 行：`RuntimeError('D3 Wave1 collector.py post-processing failed')`。
...
- `verify_collector_matches_head`，第 134 行：`RuntimeError('D3 Wave1 collector is not tracked in HEAD; commit it before formal acquisition')`。
- `verify_collector_matches_head`，第 142 行：`RuntimeError(f'D3 Wave1 collector differs from committed HEAD: worktree={worktree_sha} head={head_sha}')`。
- `verify_plan`，第 295 行：`RuntimeError(f'D3 Wave1 plan missing: {plan_path}')`。
- `verify_plan`，第 302 行：`RuntimeError(f'D3 Wave1 plan file SHA mismatch: {plan_file_sha} != {EXPECTED_PLAN_FILE_SHA256}')`。
- `verify_plan`，第 310 行：`RuntimeError(f"D3 Wave1 plan_version mismatch: {plan.get('plan_version')!r}")`。
- `verify_plan`，第 316 行：`RuntimeError(f"D3 Wave1 plan challenge_git_sha mismatch: {plan.get('challenge_git_sha')!r}")`。
- `verify_plan`，第 322 行：`RuntimeError('D3 Wave1 plan was not generated through formal code gate')`。
- `verify_plan`，第 343 行：`RuntimeError(f'D3 Wave1 plan canonical hash mismatch: embedded={embedded_canonical} recomputed={recomputed_canonical}')`。
- `verify_plan`，第 371 行：`RuntimeError(f'Plan Teacher mismatch for {key}: {actual!r} != {wanted!r}')`。
- `verify_plan`，第 379 行：`RuntimeError('D3 Wave1 plan[] missing')`。
- `verify_plan`，第 384 行：`RuntimeError(f'D3 Wave1 run count mismatch: {len(runs)} != {EXPECTED_RUNS}')`。
- `verify_plan`，第 406 行：`RuntimeError('D3 Wave1 extension_id uniqueness failure')`。
- `verify_plan`，第 414 行：`RuntimeError('D3 Wave1 extension_seed uniqueness failure')`。
- `verify_plan`，第 426 行：`RuntimeError(f'D3 Wave1 seed namespace is not the exact frozen {EXPECTED_SEED_START}..{EXPECTED_SEED_END}')`。
- `verify_plan`，第 433 行：`RuntimeError(f"Non-TRAIN_POSITIVE row leaked into D3 Wave1 plan: {row.get('extension_id')}")`。
- `verify_plan`，第 442 行：`RuntimeError(f"Forbidden source bucket in D3 Wave1: {row.get('extension_id')} {row.get('source_bucket')!r}")`。
...
| `verify_collector_matches_head` / 134 | `result.returncode != 0` | `raise RuntimeError('D3 Wave1 collector is not tracked in HEAD; commit it before formal acquisition')` |
| `verify_collector_matches_head` / 142 | `worktree_sha != head_sha` | `raise RuntimeError(f'D3 Wave1 collector differs from committed HEAD: worktree={worktree_sha} head={head_sha}')` |
...
| `verify_plan` / 295 | `not plan_path.is_file()` | `raise RuntimeError(f'D3 Wave1 plan missing: {plan_path}')` |
| `verify_plan` / 302 | `plan_file_sha != EXPECTED_PLAN_FILE_SHA256` | `raise RuntimeError(f'D3 Wave1 plan file SHA mismatch: {plan_file_sha} != {EXPECTED_PLAN_FILE_SHA256}')` |
| `verify_plan` / 310 | `plan.get('plan_version') != EXPECTED_PLAN_VERSION` | `raise RuntimeError(f"D3 Wave1 plan_version mismatch: {plan.get('plan_version')!r}")` |
| `verify_plan` / 316 | `plan.get('challenge_git_sha') != EXPECTED_PLAN_REPO_SHA` | `raise RuntimeError(f"D3 Wave1 plan challenge_git_sha mismatch: {plan.get('challenge_git_sha')!r}")` |
| `verify_plan` / 322 | `plan.get('formal_code_gate') is not True` | `raise RuntimeError('D3 Wave1 plan was not generated through formal code gate')` |
| `verify_plan` / 343 | `embedded_canonical != EXPECTED_PLAN_CANONICAL_SHA256 or recomputed_canonical != EXPECTED_PLAN_CANONICAL_SHA256` | `raise RuntimeError(f'D3 Wave1 plan canonical hash mismatch: embedded={embedded_canonical} recomputed={recomputed_canonical}')` |
| `verify_plan` / 371 | `actual != wanted` | `raise RuntimeError(f'Plan Teacher mismatch for {key}: {actual!r} != {wanted!r}')` |
| `verify_plan` / 379 | `not isinstance(runs, list)` | `raise RuntimeError('D3 Wave1 plan[] missing')` |
| `verify_plan` / 384 | `len(runs) != EXPECTED_RUNS` | `raise RuntimeError(f'D3 Wave1 run count mismatch: {len(runs)} != {EXPECTED_RUNS}')` |
| `verify_plan` / 406 | `any((not isinstance(x, str) or not x for x in ids)) or len(ids) != len(set(ids))` | `raise RuntimeError('D3 Wave1 extension_id uniqueness failure')` |
| `verify_plan` / 414 | `any((type(x) is not int for x in seeds)) or len(seeds) != len(set(seeds))` | `raise RuntimeError('D3 Wave1 extension_seed uniqueness failure')` |
| `verify_plan` / 426 | `seeds != expected_seeds` | `raise RuntimeError(f'D3 Wave1 seed namespace is not the exact frozen {EXPECTED_SEED_START}..{EXPECTED_SEED_END}')` |
| `verify_plan` / 433 | `row.get('policy_class') != 'TRAIN_POSITIVE'` | `raise RuntimeError(f"Non-TRAIN_POSITIVE row leaked into D3 Wave1 plan: {row.get('extension_id')}")` |
| `verify_plan` / 442 | `row.get('source_bucket') not in {'SEEN', 'VARIANT'}` | `raise RuntimeError(f"Forbidden source bucket in D3 Wave1: {row.get('extension_id')} {row.get('source_bucket')!r}")` |
...
| `collect_dataset` / 866 | `result.returncode != 0` | `raise RuntimeError('D3 Wave1 collector.py post-processing failed')` |
