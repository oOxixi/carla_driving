...
EXPECTED_PLAN_VERSION = "b1_d2_expansion_wave2_v1"
...
EXPECTED_RUNS = 2000
EXPECTED_SEED_START = 2_100_000
EXPECTED_SEED_END = 2_101_999
...
DEFAULT_OUTPUT_ROOT = "artifacts/b1_d2_wave2_2000_teacher_v4"
...
def verify_collector_matches_head(
    ...
    rel = Path("challenge/dataset/collect_d2_wave2.py")
    path = repo / rel
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Wave2 collector is not tracked in HEAD; commit it before "
            "formal acquisition"
        )
    ...
    if worktree_sha != head_sha:
        raise RuntimeError(
            "Wave2 collector differs from committed HEAD: "
...
def verify_plan(
    ...
    if plan.get("repo_git_sha") != EXPECTED_PLAN_REPO_SHA:
        raise RuntimeError(
            "Wave2 plan repo_git_sha mismatch: "
            f"{plan.get('repo_git_sha')!r}"
        )
...
def collect_dataset(
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Wave2 collector.py post-processing failed"
        )
