...
python -m tools.run_qwen_latency_gate \
  ...
  --dynamic-frames-dir artifacts/runtime/qwen_dynamic_frames \
  --output artifacts/reports/qwen/qwen3vl_2b_latency_gate.json
```

先从实际场景采集至少 15 个 content-unique 动态帧到上述目录。正式 gate 按文件名顺序
读取，前 5 个用于预热、后 10 个用于测量；只有该命令才可依据 P95 结果推进正确率。
...
python -m tools.run_qwen_latency_gate \
  --base-url http://127.0.0.1:8001/v1 \
  --fixed-image-diagnostic datasets/repro/frames/town03opt_multi_target_seed_20_center__baseline.png \
  --output artifacts/reports/qwen/qwen3vl_2b_fixed_image_diagnostic.json
```
