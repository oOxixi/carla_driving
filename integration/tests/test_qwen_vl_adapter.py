...
def test_action_choice_assembles_speed_without_json_generation() -> None:
    ...
    assert decision == {
        ...
        "decision_source": "QWEN_VL_CHOICE",
        "visual_valid": True,
    }
...
@pytest.mark.parametrize(
    ...
    [
        ("将速度设置为每秒五米", 5.0),
        ("把车速调为二十公里每小时", 20.0 / 3.6),
        ("请以每秒4米的速度沿当前道路直行", 4.0),
    ],
)
def test_clear_speed_instruction_corrects_adjacent_slow_down_choice(
    command: str,
    expected_mps: float,
) -> None:
    choice = qwen_adapter_module.QwenVLActionChoice(
    ...
    decision = StrictQwenVLAdapter(FakeActionBackend(choice))(
        _action_context(command)
    )
    ...
    assert decision["requires_confirmation"] is False
    assert decision["decision_source"] == "VOICE_SPEED_GROUNDED"


def test_relative_slow_down_is_not_reclassified_as_speed_setting() -> None:
    choice = qwen_adapter_module.QwenVLActionChoice(
        code="C",
        action="SLOW_DOWN",
        confidence=0.92,
    ...
    decision = StrictQwenVLAdapter(FakeActionBackend(choice))(
        _action_context("减速到每秒两米")
    )

    assert decision["action"] == "SLOW_DOWN"
    assert "target_speed_mps" not in decision
    assert decision["decision_source"] == "QWEN_VL_CHOICE"


def test_speed_setting_does_not_override_red_light_safety_rule() -> None:
    choice = qwen_adapter_module.QwenVLActionChoice(
        code="C",
        action="SLOW_DOWN",
        confidence=0.92,
    ...
    decision = StrictQwenVLAdapter(FakeActionBackend(choice))(
        _action_context("将速度设置为每秒五米", traffic_light="RED")
    )
...
def test_low_confidence_action_choice_fails_closed() -> None:
    ...
    assert decision["requires_confirmation"] is True
    assert decision["decision_source"] == "QWEN_VL_LOW_CONFIDENCE"


def test_low_confidence_speed_grounding_still_fails_closed() -> None:
    choice = qwen_adapter_module.QwenVLActionChoice(
        code="C",
        action="SLOW_DOWN",
        confidence=0.59,
...
def test_invalid_visual_speed_grounding_still_fails_closed() -> None:
    choice = qwen_adapter_module.QwenVLActionChoice(
        code="C",
        action="SLOW_DOWN",
        confidence=0.92,
    )
    context = _action_context("将速度设置为每秒五米")
    decision = StrictQwenVLAdapter(FakeActionBackend(choice))(
        replace(context, perception={**context.perception, "visual_valid": False})
    )

    assert decision["action"] == "STOP"
    assert decision["decision_source"] == "QWEN_VL_VISUAL_INVALID"
