...
def test_yolo_decoder_keeps_only_person_and_vehicle_classes() -> None:
    # 800x450 is letterboxed to 640x640: scale=.8 and vertical pad=140.
    output = _raw_yolo_output(
        (320.0, 320.0, 160.0, 160.0, 2, 0.90),  # car
        (120.0, 300.0, 60.0, 180.0, 0, 0.80),   # person
        (500.0, 300.0, 80.0, 80.0, 9, 0.99),    # traffic light: excluded in this stage
    )
