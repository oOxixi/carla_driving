...
生成 `x=0..length_m-1、y=0` 的离散直线路径；`range` 语义意味着非正长度返回空表，该演示函数自身不校验可控性。
...
生成固定 60 点的缓二次曲线路径 `(i, 0.004*i²)`，只用于无 CARLA 演示，不代表地图路线或验收基准。
...
为单个演示样例新建 Pure Pursuit 控制器，把转向步进上限设为 1.0，构造目标速度 5 m/s 的参考线，执行一帧并把 pose、steer、CTE、航向误差、目标点和 reason 打印到 stdout。
...
顺序运行直线居中、直线左右偏移和缓弯四个示例；输出中的文字预期只供人工烟测，没有断言和进程退出码判定。
