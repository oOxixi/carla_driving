...
def build_student_evaluation(
    ...
    *,
    ...
    """Build a formal Student evaluation from complete raw evidence."""
    ...
    try:
        ...
        raise EvaluationArtifactError(
            f"cannot build Student metric evidence: {error}"
        ) from error
    ...
    for name in GATE_METRICS:
        ...
        if denominator <= 0 or value is None:
            raise EvaluationArtifactError(
                f"formal Student evaluation has empty denominator "
                f"for metric {name!r}"
            )
        ...
        if (
            ...
            raise EvaluationArtifactError(
                f"formal Student metric {name!r} must be in [0, 1]"
            )
