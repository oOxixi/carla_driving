"""Dependency-light ONNX detection for frame-aligned CARLA RGB images.

ONNX Runtime is imported lazily so controller-only tests remain lightweight.
The decoder supports common Ultralytics YOLO ``[1,84,N]``/``[1,N,84]`` and
end-to-end ``[N,6]`` outputs and retains only road users needed by C/D.
"""
...
def carla_rgb_array(measurement: Any) -> np.ndarray:
    """Convert a CARLA BGRA payload (or test RGB array) to uint8 RGB."""
...
class OnnxYoloDetector:
    ...
    def detect_rgb(self, image_rgb: np.ndarray) -> tuple[DetectedObject, ...]:
        ...
        tensor, transform = _letterbox(
            image.astype(np.uint8, copy=False), self._input_width, self._input_height,
        )
        try:
...
def driving_corridor_detections(
    ...
    *,
    ...
    """Select plausible ego-lane road users without pretending lane segmentation."""
